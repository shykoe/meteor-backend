...
Meteor.methods({
  ...
  'agent.order.accept': (id, userName) => {
    ...
    const result = Orders.update({ _id: id }, {
      $set: {
  ...
  'tester.orders.get':(userName, role, page, perpage, field, order) =>{
    ...
    const skipped = ( parseInt(page) - 1 ) * parseInt(perpage);
    const sortOrder = order == 'ASC' ? 'asc':'desc';
    return Orders.find({'tester':userid},{skip:parseInt(skipped), limit:parseInt(perpage), sort:[[ field, sortOrder ]] }).fetch();
  },
  ...
  'orders.get':(page, perpage, field, order, filter)=>{
    ...
    const sortOrder = order == 'ASC' ? 'asc':'desc';
    if(filter.status){
      return Orders.find({status:{$in:filter.status}},{skip:parseInt(skipped), limit:parseInt(perpage), sort:[[ field, orderid ]] }).fetch();
    }
    return Orders.find({},{skip:parseInt(skipped), limit:parseInt(perpage), sort:[[ field, sortOrder ]] }).fetch();
  },
  ...
  'agent.allorder.get': ( page, perpage, field, order) => {
    ...
    x=Orders.find({},{skip:parseInt(skipped), limit:parseInt(perpage), sort:[order]}).fetch();
    return x;
  },

  'agent.allorder.getFilter': ( page, perpage, field, order,myfilter) => {
    const skipped = ( parseInt(page) - 1 ) * parseInt(perpage);
    x=Orders.find(myfilter,{skip:parseInt(skipped), limit:parseInt(perpage), sort:[order]}).fetch();
    return x;
  }
});
